...
describe('MeasurementForm', () => {
    test('validates height input', () => {
        render(<MeasurementForm />);
        const heightInput = screen.getByLabelText(/height/i);

        fireEvent.change(heightInput, {target: {value:'100'}});

        expect(screen.getByText('Height must be between 130 and 230cm')).toBeInTheDocument();
    })
    ...
    //     fireEvent.change(heightInput, { target: { value: '100'}});
    //     const errorElement = screen.getByTestId('height-error');
    //     expect(errorElement.textContent).toBe('height must be between 130 and 230');
